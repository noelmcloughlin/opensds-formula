  ...
  file.directory:
    ...
    - clean: True
              ...
              {%- if 'subdirs' in s.sushi.plugin and s.sushi.plugin.subdirs is iterable %}
                  {%- for dir in s.sushi.plugin.subdirs %}

sodafoundation sushi plugin config-repo {{ id }} copy {{ dir }} to workdir:
  cmd.run:
    - unless: test -d {{ s.dir.sushi }}/nbp/{{ id }}/{{ dir }}
                      {%- if id|lower == 'provisioner' %}
    - name: cp -rp {{golang.go_path}}/src/github.com/sodafoundation/nbp/sodafoundation-{{id}}/{{dir}} {{s.dir.sushi}}/nbp/{{id}}/
    - onlyif: test -d {{ golang.go_path }}/src/github.com/sodafoundation/nbp/sodafoundation-{{ id }}/{{ dir }}
                      {%- else %}
    - name: cp -rp {{golang.go_path}}/src/github.com/sodafoundation/nbp/{{id}}/{{dir}} {{s.dir.sushi}}/nbp/{{id}}/
    - onlyif: test -d {{ golang.go_path }}/src/github.com/sodafoundation/nbp/{{ id }}/{{ dir }}
...
sodafoundation sushi plugin config {{ id }} set sodafoundationendpoint in external {{ file }} file:
  file.replace:
    ...
    - repl: '  sodafoundationendpoint: {{ s.auth.daemon.auth.endpoint_ipv4 }}:{{ s.auth.daemon.auth.endpoint_port }}'
...
sodafoundation sushi plugin config {{ id }} set sodafoundationauthstrategy in external {{ file }} file:
  file.replace:
    ...
    - repl: '  sodafoundationauthstrategy: {{ s.hotpot.conf.let.auth_strategy }}'
    - onlyif: test -f {{ s.dir.sushi }}/nbp/{{ id }}/{{ file }}
